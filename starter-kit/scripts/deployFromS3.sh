...
export appname=$(grep appname deployment.properties|cut -d'=' -f2)
export version=$(grep version deployment.properties|cut -d'=' -f2)
export build=$(grep build deployment.properties|cut -d'=' -f2)
mkdir deployments
