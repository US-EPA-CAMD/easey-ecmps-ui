...
export const MonitoringPlanEvaluationReport = ({
    ...
    useEffect(() => {
        if (!dataLoaded) {
            mpApi.getMonitoringPlansEvaluationReportData(monitorPlanId).then((res) => {
                let reportResults = res.data
                reportResults.mpReportResults = fs.getMonitoringPlansEvalData(reportResults.mpReportResults);
                    ...
                    <div className="padding-x-3" id="printArea">
                        ...
                        <div className="grid-row clearfix float-left">
                            ...
                            <div className="grid-col text-bold text-left display-inline-block text-nowrap padding-top-2">
                                {reportData.facilityId}<br />
                                {facilityAdditionalName}<br />
                                {reportData.state}<br />
                                {reportData.countyName}<br />
                            </div>
                        </div>
