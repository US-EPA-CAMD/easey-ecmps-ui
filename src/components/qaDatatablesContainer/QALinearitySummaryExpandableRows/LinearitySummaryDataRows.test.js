...
import LinearitySummaryDataRows from "./LinearitySummaryDataRows";
...
const componentRenderer = (locId, testSummaryId) => {
  ...
  return render(
    <Provider store={store}>
      <LinearitySummaryDataRows {...props} />
    </Provider>
  );
};
...
test.skip("when remove button on a row is clicked then that row is deleted from the table", async () => {
  ...
  render(<LinearitySummaryDataRows {...props} />)

  // does not work in combination with screen.getAllByRole(button)
  // waitForElement(() => render(<LinearitySummaryDataRows {...props} />))
