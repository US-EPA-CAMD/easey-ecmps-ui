...
// value in data => [0] api label, [1] our UI label, [2] value
const ModalDetails = ({ modalData, data, cols, title, viewOnly }) => {
      ...
      <div className="grid-container margin-bottom-2">
        ...
        <div>
          {items.map((item,index) => {
            return <div key={index} className="grid-row padding-top-2">{item}</div>;
          })}
        </div>
      </div>
    </div>
