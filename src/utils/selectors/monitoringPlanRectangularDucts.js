...
export const getMonitoringPlansRectangularDuctsTableRecords = (totalData) => {
  ...
  data.forEach((el) => {
    const beginDate = formatDateString(el.wafBeginDate.toString())
    const beginHour =
      el.wafBeginHour !== null ? el.wafBeginHour.toString() : "";
    const endDate = formatDateString(el.wafEndDate.toString())
    const endHour = el.wafEndHour !== null ? el.wafEndHour.toString() : "";
    const wafDeterminationDate = formatDateString(el.wafDeterminationDate.toString())
